...
export default function StepEditor({ steps, onChange }: StepEditorProps) {
  ...
  function updateStep(index: number, field: keyof WorkflowStep, value: string | Record<string, unknown>) {
    const updated = [...steps]
                  ...
                  {step.type === 'loop' && (
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateStep(index, 'loopConfig', parseLoopConfig(e.target.value) || undefined)}
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-white text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                        rows={3}
