/**
 * End-to-End Migration Test - Minimal
 *
...
describe('E2E Migration Test - Direct Database', () => {
  ...
  before(async () => {
    ...
    let user = await prisma.user.findFirst({
      where: { emailAddress: 'test@clawdeck.dev' },
    })

    if (!user) {
      user = await prisma.user.create({
        data: {
          emailAddress: 'test@clawdeck.dev',
          agentAutoMode: true,
        },
      })
    }
  ...
  it('should verify tasks in PostgreSQL directly via raw query', async () => {
    const result = await prisma.$queryRaw`
      SELECT id, name, status, priority, tags, "boardId", "userId"
      FROM "Task"
    ...
    result.forEach((t) => {
      console.log(`   - Task ${t.id.toString()}: ${t.name} [${t.status}]`)
    })
  })
  ...
  it('should add activity log for task (UPDATE: Activity)', async () => {
    const taskId = testTaskIds[3]
    ...
    assert.ok(activities.length > 0, 'Should have activity logs')
  ...
  it('should display E2E test summary', async () => {
    ...
    console.log('  ✅ READ: Single task, All tasks, Filter by status, Raw query')
    console.log('  ✅ UPDATE: Assign, Claim, Modify, Complete, Unclaim, Unassign, Activity')
    ...
    console.log('  - Activity logs created: 1')
